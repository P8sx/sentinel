...
esp_err_t ghota_check(ghota_client_handle_t *handle)
{
    ...
    if (err == ESP_OK)
    {
        ESP_LOGD(TAG, "HTTP GET Status = %d, content_length = %lu", esp_http_client_get_status_code(client), (ulong)esp_http_client_get_content_length(client));
    }
    else
    {
...
static esp_err_t validate_image_header(esp_app_desc_t *new_app_info)
{
    ...
    ESP_LOGD(TAG, "Current partition %s type %d subtype %d (offset 0x%08x)", running->label, running->type, running->subtype, (uint)running->address);
    const esp_partition_t *update = esp_ota_get_next_update_partition(NULL);
    ESP_LOGD(TAG, "Update partition %s type %d subtype %d (offset 0x%08x)", update->label, update->type, update->subtype, (uint)(update->address));
...
esp_err_t ghota_storage_update(ghota_client_handle_t *handle) {
    ...
    ESP_LOGD(TAG, "Storage Partition %s - Type %x Subtype %x Found at %x - size %d", handle->storage_partition->label, handle->storage_partition->type, handle->storage_partition->subtype, (uint)(handle->storage_partition->address), (uint)(handle->storage_partition->size));
...
esp_err_t ghota_start_update_timer(ghota_client_handle_t *handle) {
    ...
    if ( timer == NULL) {
        ...
        if (xTimerStart(timer, 0) != pdPASS) {
            ...
            ESP_LOGI(TAG, "Started Update Timer for %i Minutes", (uint)handle->config.updateInterval);
        }
    }
    return ESP_OK;
}
