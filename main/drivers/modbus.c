...
void modbus_task(void *arg)
{
    ...
    while (true)
    {
        ...
        else if ((reg_info.type & MB_EVENT_HOLDING_REG_WR || reg_info.type & MB_EVENT_HOLDING_REG_RD) && (uint8_t *)&holding_reg_params.input_action == reg_info.address)
        {
            ...
            holding_reg_params.input_action = 0;
            portEXIT_CRITICAL(&param_lock);
