...
static void mqtt_uptime_timer_callback(TimerHandle_t xTimer)
{
    ...
    {
        esp_mqtt_client_publish(mqtt_client, mqtt_availability_topic, "1", 0, 1, 0);
    }
    availability_update++;
}
...
#define MQTT_CFG_SIZE 640
    ...
    case MQTT_EVENT_DATA:
        ...
        {
            xQueueSend(wing_action_queue, &WING_CMD((strncmp("OPEN", event->data, event->data_len) == 0) ? OPEN : (strncmp("CLOSE", event->data, event->data_len) == 0) ? CLOSE : STOP, RIGHT_WING), pdMS_TO_TICKS(500));
        }
        else if (strncmp(event->topic, mqtt_left_wing_cmd_topic, event->topic_len) == 0)
        {
            xQueueSend(wing_action_queue, &WING_CMD((strncmp("OPEN", event->data, event->data_len) == 0) ? OPEN : (strncmp("CLOSE", event->data, event->data_len) == 0) ? CLOSE : STOP, LEFT_WING), pdMS_TO_TICKS(500));
        }
        else if (strncmp(event->topic, mqtt_both_wing_cmd_topic, event->topic_len) == 0)
        {
            xQueueSend(wing_action_queue, &WING_CMD((strncmp("OPEN", event->data, event->data_len) == 0) ? OPEN : (strncmp("CLOSE", event->data, event->data_len) == 0) ? CLOSE: STOP, BOTH_WING), pdMS_TO_TICKS(500));
        }
